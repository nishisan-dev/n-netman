...
func TestLoader_Load_VagrantStyleV2(t *testing.T) {
	// Mirrors the config generated by the Vagrant lab (v2, root peers as a
	// 4-space indented sequence, two overlays with distinct tables).
	yaml := `
version: 2
node:
  id: "host-a"
...
`
	cfg, err := NewLoader().Load([]byte(yaml))
	if err != nil {
		t.Fatalf("expected Vagrant-style v2 config to load, got: %v", err)
	}
	if got := len(cfg.GetPeers()); got != 2 {
		t.Fatalf("expected 2 root peers, got %d", got)
	}
}
